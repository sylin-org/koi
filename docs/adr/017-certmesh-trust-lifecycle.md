...
**Status:** Accepted — **Implemented** (all four phases: 1a cert profiles + CA-held policy; 1b member-pull rotate-key renewal; 2 signed monotonic trust bundle + sequenced single-writer roster + boundary revocation; 3 pinned-fingerprint bootstrap + mDNS fp advertise + anchor self-heal; 4 machine binding + full failure audit + promote key/cert check + cert_path serde-skip + RFC 1123 validator + pond/K2 scrub + persisted rate limiter). The one carried residual is a **signed live CA re-key transition** (F5's re-key arm) — deferred to a future ADR, since no CA re-key path exists today and the fail-safe + re-enroll recovery is the secure interim. (operator-ratified key decisions 2026-06-18)
**Date:** 2026-06-18
...
4. **Custody, observability, hygiene. ✅ Implemented.** F11 machine-binding recorded at `create`; auto-unlock refuses (boots locked + audits `auto_unlock_refused_machine_changed`) when the machine fingerprint changed, checked in the **real** boot path (`koi_compose::init_certmesh_core`) before the vault is touched. F9/F14 every trust decision — including all enrollment denials, the mTLS revocation boundary, `unlock_failed`, `self_enroll`, and `roster_persist_failed` — is audited *before* the error returns; the promote handler verifies the decrypted CA key's public key matches the transferred cert. F13 `RosterMember.cert_path` is `#[serde(skip)]` (derived from paths where needed). F15 one shared RFC 1123 hostname validator at every SAN/dir site (also closes audit-log injection). F16 retired `pond_initialized → ca_initialized` and `pond_ceremony → init_ceremony`, scrubbed the consumer-name comment (K2). F7 the TOTP enrollment lockout is wall-clock + persisted (`totp-throttle.json`) so a restart can't reset it; invites stay unthrottled.

...
| W6 | DNS serve + resolve | UDP/TCP 53 + cross-host `dig` | Daemon serves its authoritative `.internal` zone on the catalog DNS port (18553), verified cross-host by `dig` and locally by `Resolve-DnsName` — same contract as the Linux lanes. Standard-port 53 is a documented limitation on workstation Windows: system services (ICS/SharedAccess) hold 53 with reuse semantics, so a bind "succeeds" while receiving no traffic (measured 2026-08-26). Koi is a zone contributor, never the system resolver. | not started |

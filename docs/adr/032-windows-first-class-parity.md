...
| W5 | mDNS announce/browse | mdns-sd multicast | **Announce: green** — Windows daemon announces via the announce API; avahi-browse on test01 discovers it through the standard stack (standards conformance proven; Koi is the lab's only koi-side mDNS participant — Linux daemons deliberately skip per ADR-030). **Browse: product defect found** — the browser cache never populates on Windows (live events flow, cache write path dead; reproduced on the standing SYSTEM service); live-event surfaces (workbench pond) unaffected. Scenario `windows-mdns` lands as the regression test; ticket filed. | partial — announce green, browse defect ticketed |

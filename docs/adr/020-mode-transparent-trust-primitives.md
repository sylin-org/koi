...
| **P2 — Envelope** ◑ mostly landed | `Envelope`/`Sig`/`Assurance` wire types + `sign`/`verify` (carry-cert; one-identity-door; freshness sub-field; ±300 s window; version-selects-construction; 7 adversarial tests) + koi-crypto leaf-key signing + embedded-facade `sign`/`verify` ✅. **`require_auth` axum gate remaining.** | M |

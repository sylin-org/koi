...
| **P4 — Same-port dial + posture events** ✅ landed (1 sub-item tracked) | `CertmeshCore::watch_posture()` (a coalesced `tokio::sync::watch<Posture>` republished from the four identity-mutating sites) + `KoiEvent::PostureChanged` (degrade as loud as upgrade); the same-port dial — `koi_certmesh::serve` per-connection primitives (`serve_plain`/`serve_mtls`/`AdaptiveServerConfig`, 1-byte ClientHello sniff) + `koi_embedded::serve_adaptive` supervisor that binds **one** socket and dispatches each connection against the posture at accept time, so a flip never drops an in-flight connection and refusals are loud (plaintext→secure / TLS→Open); `KoiHandle::serve` + `participate()` (the 3-line trusted service: ensure_identity + posture-stamped announce kept current across flips + serve). Tests incl. a live Open→secure flip on one running listener. **Remaining sub-item:** make the *daemon's* fixed 5642 mTLS listener posture-reactive too (the ADR-016 §2 startup-gating fix) — deferred as a focused change: it touches two boot paths (`daemon.rs` + the Windows service path) and the ACME listener's shared self-enrollment, so it warrants its own tested increment rather than riding this phase. | **L** |

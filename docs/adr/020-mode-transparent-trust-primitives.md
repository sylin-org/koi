...
) -> Result<SocketAddr, KoiError>;
```

- **Readiness:** the call acquires one real listener and returns its exact address; bind failure is
  returned before a serving generation exists.
- **Open:** plain HTTP on the bound port.
- **Authenticated/Confidential:** mTLS-HTTPS on the **same** bound port, injecting `ClientCn` (reuses `koi_certmesh::mtls::serve`).
- **On posture transition:** the same port **upgrades** plain↔mTLS — no daemon restart, **no dropped connections**.

Mechanism (new): a `tokio::sync::watch<Posture>` published by `CertmeshCore` (derived from existing CA create/unlock/lock/destroy events) drives a **listener supervisor** in `koi-embedded`. **Critically — this is Istio PERMISSIVE mode (§13), so the flip must not be a hard cutover:** one acceptor retains the socket and dispatches each new connection by its TLS ClientHello and the current posture. Existing connections drain under the posture at which they were accepted. A naive close-and-rebind would drop in-flight connections — the exact failure operators fear. The same posture-watch makes Koi's *own* mTLS listener posture-reactive, resolving ADR-016 §2's startup-gating bug. **Per-connection trust state is always loudly observable** (`plaintext | mTLS(cn=…)` on status/dashboard — a "padlock on the wire"); an operator must never `tcpdump` to learn whether traffic is encrypted.
...
- `koi-embedded` — the ergonomic facade: `ensure_identity`, typed `discover`/`Peer`, and the readiness-fenced posture-watching `serve` boundary. This is where the consumer's single code path and its listener/discovery ownership live.

### 12. Phasing

| Phase | Content | Size |
|---|---|---|
| **P1 — Posture + identity** ✅ landed | `Posture`/`PostureLevel` + the `posture()` oracle; unified `Identity` + `RenewalHealth` (derived schedule) + read-only `local_identity()`; idempotent `ensure_identity()`; embedded-facade passthroughs. (`client_for`/the Certmesh status notification/`participate()` had forward deps → re-homed to P3/P4; the 2/3-% renewal trigger + graduated/first-failure events land with the renewal loop.) | S–M |
| **P2 — Envelope + require_auth** ✅ landed | `Envelope`/`Sig`/`Assurance` wire types + `sign`/`verify` (carry-cert; one-identity-door; freshness sub-field; ±300 s window; version-selects-construction; 7 adversarial tests) + koi-crypto leaf-key signing + embedded-facade `sign`/`verify` + `require_auth` posture-aware route gate (no-op in Open, mTLS-CN-or-401 when secure; 3 tests). | M |
| **P3 — Discovery + fleet legibility + client_for** ✅ landed | `koi_common::peer::Peer` (typed, posture-carrying view of a `ServiceRecord`) + `MdnsHandle::discover` (snapshot over a sane default window, both embedded+remote); `CertmeshCore::client_for(&Peer)` → `PeerClient` (plain HTTP to an Open peer / mTLS-with-our-leaf+pinned-CA to a secure peer, one code path; loud errors on Open→secure and cross-mesh `fp` mismatch — not opaque TLS failures); `fp=`/`posture=`/`expires=` stamped into the CA `_certmesh._tcp` **and** every node's `_http._tcp` announce (absolute RFC3339 expiry); shared `peer::stamp`/`parse` so the TXT vocabulary is one. `mtls` request plumbing refactored to one generic `drive_request` (renewal path unchanged, green). Hints stay advisory (verify adjudicates). 9 (certmesh) + 18 (common) + 4 (embedded) tests + 2 live round-trips. | M |
| **P4 — Same-port dial + posture notification** ✅ landed | `CertmeshCore::watch_status()` exposes the authoritative latest value and `KoiEvent::CertmeshStatusChanged { revision, posture }` forwards an explicitly coalescing notification. It is not semantic history and revisions may skip; true Certmesh facts continue through `CertmeshEvent`. The same-port dial uses `koi_certmesh::serve` per-connection primitives (`serve_plain`/`serve_mtls`/`AdaptiveServerConfig`, 1-byte ClientHello sniff) behind the sole public `KoiHandle::serve` path. That async boundary prebinds **one** real socket, returns its actual address only after readiness, and retains the adaptive worker under composition ownership. `participate()` adds a transactionally established posture-stamped mDNS registration on that exact port before it reports success; listener and registration then live and stop as one generation. A posture flip never drops an in-flight connection and refusals are loud (plaintext→secure / TLS→Open). **The daemon's fixed 5642 mTLS + 5643 ACME listeners are now posture-reactive too** (the ADR-016 §2 startup-gating fix), via one shared trust-plane supervisor that both boot paths call. Tests cover live Open→secure transition, occupied-port refusal before advertisement, exact ephemeral-port publication, cancellation, rollback, and the real-binary two-daemon post-boot transition. **Follow-up (two-box hardware run, 2026-06-21):** the `_certmesh._tcp` CA discovery announce was folded into the same trust-plane supervisor, published when the CA appears and withdrawn when it is destroyed. | **L** |
| **P5 — `seal`/`open` passthrough** ✅ landed | `koi_common::sealed::Sealed` (version-tagged: v0 passthrough / v1 group-key reserved) + `Confidentiality` (type-level — passthrough can't be mistaken for encrypted) + `Opened` (payload + assurance + confidentiality; Debug redacts payload). `seal`/`open` **built on** the P2 `sign`/`verify` machinery (a v0 Sealed *is* a signed Envelope + version tag) — no re-implemented crypto; `open` never yields bytes from a rejected envelope, dispatches on the version (anti-downgrade extension point), and a one-time `warn!` + `/v1/status` `seal: passthrough\|groupkey` (and a CLI `Seal:` line) make the un-encrypted state observable, not silent. The reserved K3-distinct HKDF label `b"koi-seal-group-v1"` is staked in koi-crypto for the v1 rung (frozen `pond-*` labels untouched). | S–M |
| **P6 — diagnose() + testkit + wire contract** ✅ landed | **diagnose()** — `koi_common::diagnosis::TrustDiagnosis` + `CertmeshCore::diagnose` (reuses posture/`Identity`/`RenewalHealth`/roster): distinct named checks (posture, identity, integrity-chains-to-CA, self-revocation, renewal-health, ca-trust-install, clock) each with an exact remedy; **non-zero exit only on RED**; `koi trust diagnose [--fix]` (extends the existing trust command; `--fix` installs the mesh CA, reusing `install_cert`) + `GET /v1/certmesh/diagnose` + embedded facade — all one logic. **Honesty rule:** the ca-trust-install check states the os-truststore-can't-query limitation instead of a fake "installed" (mkcert #182). **testkit** — `koi_embedded::testkit::{open,secured}` (real embedded daemons, no Docker; shipped as a module, not a feature) + the `both_postures.rs` **same-code-both-postures CI gate**. **Wire contract** — `docs/reference/trust-protocol.md` (Posture/Envelope/Sealed/dual-mode handshake/diagnose, language-neutral) realizes the **D7 amendment**. **Conformance vectors** — `docs/reference/vectors/trust-vectors.json` (certless, deterministic, graded `expect`) + a validator that keeps them in sync with the impl + a generator. *Scoped per "less but more meaningful":* `#[koi::test]` proc-macro sugar deferred (the both-postures capability ships via testkit + a loop); clock-skew-vs-peers and cross-language signed-verify vectors deferred (the certless vectors + the reference impl's adversarial tests cover the contract). | M–L |
| **P7 — (stretch) deterministic simulator** ✅ landed | sans-IO + `proptest` LAN-trust state machine; doubles as Koi's own trust state-machine test backbone. Shipped as `crates/koi-certmesh/tests/trust_sim.rs` — **internal-only** (Koi's own test backbone), not a consumer-facing tool (resolves Open Question #3). | M |
...
**`participate()` + posture legibility (Tailscale's `up`, plus the legibility it lacks)** — folded into P1: `participate()` (best-effort `ensure_identity` + real listener acquisition + acknowledged announce on its actual port + owned adaptive serving, maintained across flips/renewals) is the "3-line trusted service." A failed bind or initial announcement rolls the whole startup generation back; a host observation failure remains an error rather than an `unknown` identity. `KoiEvent::CertmeshStatusChanged { revision, posture }` makes the current posture observable in both upgrade and degrade directions. Because its source is a coalescing latest-value watch, it deliberately does not claim `from → to` semantic history; consumers reconcile from the advertised revision or `certmesh().status()`.
...
3. **Deterministic simulator (P7) — RESOLVED: internal-only.** Shipped as `crates/koi-certmesh/tests/trust_sim.rs`, Koi's own trust state-machine test backbone — not a consumer-facing tool. (The replay-window default is now decided: **±300 s**, §13.)

...
| **P4 — Same-port dial + posture events** ✅ landed | `CertmeshCore::watch_posture()` (a coalesced `tokio::sync::watch<Posture>` republished from the four identity-mutating sites) + `KoiEvent::PostureChanged` (degrade as loud as upgrade); the same-port dial — `koi_certmesh::serve` per-connection primitives (`serve_plain`/`serve_mtls`/`AdaptiveServerConfig`, 1-byte ClientHello sniff) + `koi_embedded::serve_adaptive` supervisor that binds **one** socket and dispatches each connection against the posture at accept time, so a flip never drops an in-flight connection and refusals are loud (plaintext→secure / TLS→Open); `KoiHandle::serve` + `participate()` (the 3-line trusted service: ensure_identity + posture-stamped announce kept current across flips + serve). **The daemon's fixed 5642 mTLS + 5643 ACME listeners are now posture-reactive too** (the ADR-016 §2 startup-gating fix), via one shared `crate::adapters::trust_plane` supervisor that both boot paths (`daemon.rs` + the Windows service) call — removing ~100 lines of duplication and a latent parity defect (the Windows path had silently omitted ACME). Tests: a live Open→secure flip on one running listener (embedded) **and** the real-binary two-daemon test asserting the 5642 listener comes up after a post-boot `create` with no restart. **Follow-up (two-box hardware run, 2026-06-21):** the `_certmesh._tcp` CA discovery announce was the same class of startup-gated bug (an Open-booted node that ran `create` never advertised `fp=`/`posture=` until restart) — it is now folded into the **same** `trust_plane` supervisor, published when the CA appears and withdrawn when it is destroyed, on the same `watch_posture` signal. | **L** |

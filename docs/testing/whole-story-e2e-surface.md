...
| status / host / dashboard / MCP | **Full** | Two concurrent real daemons each prove the centralized ten-rung status/dashboard projection, host JSON, public server card, DAT refusal, all 11 MCP tools, all four resources, and a live read |
...
- **Runtime without Docker** — implemented by a test `RuntimeBackend` admitted through `RuntimeCore::start_with_backend`, the same provider lifecycle real backends use. The always-on `koi-compose` story proves initial snapshot reconciliation, derived mDNS/DNS/health/live-proxy state, lifecycle observation, and exact reversal without a production event-injection shortcut. Physical Linux runs prove the same reconstruction after restarting Koi while a real container remains running. Real containers remain Tier 2 / Docker-available CI; a separate run-owned Unix-relay lane now proves Docker event-stream disconnect/reconnect in both physical role directions.

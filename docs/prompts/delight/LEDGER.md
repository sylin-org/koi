...
| `R06/windows-recovery-tail` | cachyos-linux | windows | Installed exact ccee0fc; elevated independent recovery guard then service-loss/recovery only; no rebuild/reinstall; [procedure](reports/R06-renderer-decision.md#r06windows-recovery-tail) | requested | Requires elevated operator session; prior positive proof preserved |

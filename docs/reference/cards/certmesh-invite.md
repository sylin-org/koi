...
- **Hygiene (F13/F15/F16).** `RosterMember.cert_path` is `#[serde(skip)]` — the CA host's local path never leaks into `roster.json`. One shared **RFC 1123** validator gates every hostname that becomes a SAN or a `certs/` directory (and, by construction, blocks audit-log injection). The legacy "pond" term is retired from the audit/API surface (`pond_initialized → ca_initialized`, `pond_ceremony → init_ceremony`) and the consumer-name comment in the roster is scrubbed (STACK-0001 K2).

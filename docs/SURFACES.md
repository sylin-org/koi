...
| mcp (stdio + in-process Streamable HTTP for AI agents) | koi-mcp suite + koi http-session test | 2026-06-15 | koi-mcp + koi tests (ci.yml) | stdio `koi mcp serve` plus an in-process Streamable HTTP transport at /v1/mcp (token-authed on every method incl. the SSE GET; not in /openapi.json; --no-mcp-http gate; reported as the mcp_http status field). 11 tools + 4 resources (read = snapshot on both transports; resources/updated deltas on the in-process path). Tests: stdio session (tools + resources/list), full HTTP session (initialize through resources/read via a mock source), auth carve-out, change-routing units. Live tool calls against a running daemon still not exercised end-to-end |

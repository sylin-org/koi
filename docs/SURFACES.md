...
| certmesh (CA / enroll / roster / renew) | koi-certmesh suite (254 tests) | 2026-06-19 | koi-certmesh tests (ci.yml) | The claimed invention; revocation is roster-only (no CRL/OCSP). ADR-017 phases 1–3 shipped: CSR-only issuance + member-pull rotate-key renewal, signed monotonic trust bundle + sequenced single-writer roster + boundary revocation, and pinned-fingerprint bootstrap (invite carries CA fp → preflight + install hard-fail, closing the invite-join MITM gap) + `_certmesh._tcp` fp= advertise + member anchor self-heal |

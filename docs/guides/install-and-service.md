...
Both installers honor the same knobs as environment variables: `KOI_VERSION` (pin a release tag, e.g. `v0.7.0`), `KOI_INSTALL_DIR` (install location), and `KOI_NO_MODIFY_PATH` (skip the PATH change).

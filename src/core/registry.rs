...
impl Registry {
    ...
    pub fn force_revive(&self, id: &str) -> Result<()> {
        ...
        match &reg.state {
            ...
            RegistrationState::Alive => Err(KoiError::NotDraining(id.to_string())),
        }
    }
    ...
    pub fn resolve_prefix(&self, prefix: &str) -> Result<String> {
        ...
        match matches.len() {
            0 => Err(KoiError::RegistrationNotFound(prefix.to_string())),
            1 => Ok(matches[0].clone()),
            _ => Err(KoiError::AmbiguousId(prefix.to_string())),
        }
    }
    ...
    pub(crate) fn force_drain_at(&self, id: &str, now: Instant) -> Result<()> {
        ...
        match &reg.state {
            ...
            RegistrationState::Draining { .. } => Err(KoiError::AlreadyDraining(id.to_string())),
        }
    }
...
mod tests {
    ...
    fn force_drain_already_draining_returns_error() {
        ...
        assert!(matches!(
            reg.force_drain("abc"),
            Err(KoiError::AlreadyDraining(_))
        ));
    }
    ...
    fn resolve_prefix_errors_on_not_found() {
        ...
        assert!(matches!(
            ...
            Err(KoiError::RegistrationNotFound(_))
        ));
    }

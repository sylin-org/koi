...
fn kill_transient_service(
    ...
    let unit = transient_unit_name(run_id);
    let command = format!(
        "set -eu; guard() {{ echo \"koi-lab service-lifecycle guard failed: $1\" >&2; exit 71; }}; lock_owner=$(cat {lock_dir}/owner 2>/dev/null) || guard \"lock owner file {lock_dir}/owner is missing\"; test \"$lock_owner\" = {} || guard \"lab lock held by '$lock_owner'\"; run_owner=$(cat {run_dir}/owner 2>/dev/null) || guard \"run owner file {run_dir}/owner is missing\"; test \"$run_owner\" = {} || guard \"run dir owned by '$run_owner'\"; test \"$(cat {run_dir}/systemd-unit)\" = {unit}; test \"$(systemctl show --value -p MainPID {unit})\" = {expected_pid}; test \"$(readlink -f /proc/{expected_pid}/exe)\" = {run_dir}/koi; sudo -n systemctl kill --kill-whom=main --signal=KILL {unit}",
        run_id.as_str(),
        run_id.as_str(),
    );
    lab.run_remote_checked(node, &command)?;

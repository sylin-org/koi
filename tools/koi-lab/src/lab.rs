...
impl Lab {
    ...
    fn launch_run_daemon(
        ...
        let command = format!(
            "set -eu; guard() {{ echo \"koi-lab daemon-launch guard failed on {node_id}: $1\" >&2; exit 71; }}; lock_owner=$(cat {lock_dir}/owner 2>/dev/null) || guard \"lock owner file {lock_dir}/owner is missing\"; test \"$lock_owner\" = {run_id_str} || guard \"lab lock held by '$lock_owner', expected {run_id_str}\"; run_owner=$(cat {run_dir}/owner 2>/dev/null) || guard \"run owner file {run_dir}/owner is missing\"; test \"$run_owner\" = {run_id_str} || guard \"run dir owned by '$run_owner', expected {run_id_str}\"; test ! -e {run_dir}/daemon.pid || guard \"stale daemon.pid under {run_dir}\"; i=0; while ss -H -lntup | grep -Eq ':({guarded_ports}) ' && test \"$i\" -lt 24; do sleep .5; i=$((i+1)); done; if ss -H -lntup | grep -Eq ':({guarded_ports}) '; then guard \"guarded ports busy after 12s: {guarded_ports}\"; fi; {prepare}; setsid -f sh -c 'echo $$ > {run_dir}/daemon.pid; exec env KOI_DATA_DIR={run_dir}/data KOI_DNS_ZONE=internal XDG_RUNTIME_DIR={run_dir}/runtime KOI_NO_CREDENTIAL_STORE=1{runtime_env} {run_dir}/koi --daemon --port {http_port} --http-bind 0.0.0.0 --mtls-port {mtls_port} --acme-port {acme_port} {profile_args} >>{run_dir}/daemon.log 2>&1'",
            node_id = node.id(),

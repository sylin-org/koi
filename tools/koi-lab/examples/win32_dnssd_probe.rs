...
mod windows {
    ...
    use windows_sys::Win32::NetworkManagement::Dns::{
        DnsServiceBrowse, DnsServiceBrowseCancel, DnsServiceConstructInstance,
        DnsServiceDeRegister, DnsServiceFreeInstance, DnsServiceRegister, DnsServiceResolve,
        DNS_RECORDW, DNS_SERVICE_BROWSE_REQUEST, DNS_SERVICE_CANCEL, DNS_SERVICE_INSTANCE,
    ...
    fn evidence(step: &str, fields: &[(&str, serde_json::Value)]) {
        let map: serde_json::Map<String, serde_json::Value> = fields
            ...
            .collect();
        println!("{}", serde_json::json!({"step": step, "fields": map}));
    }
    ...
    fn register(
        ...
        if instance.is_null() {
            evidence(
                "register",
                &[("error", "DnsServiceConstructInstance returned null".into())],
            );
            return None;
        }
        ...
        unsafe extern "system" fn register_complete(
            ...
            let sender =
                unsafe { &*(query_context as *const Context<(u32, *const DNS_SERVICE_INSTANCE)>) };
        ...
        if call_status == DNS_STATUS_SUCCESS_DWORD || call_status == DNS_REQUEST_PENDING {
            match rx.recv_timeout(WAIT_TIMEOUT) {
                ...
                Err(_) => evidence(
                    "register",
                    &[("error", "completion callback timed out".into())],
                ),
            }
        } else {
                ...
                Ok((status, returned)) => unsafe {
                    if !returned.is_null() {
                        ...
                        if !(*returned).ip6Address.is_null() {
                            let bytes =
                                std::slice::from_raw_parts((*returned).ip6Address as *const u8, 16);
                            service.ipv6.push(
    ...
    fn hold_mode(secs: u64) {
        ...
        let host_name = format!(
            ...
            std::env::var("COMPUTERNAME")
                .unwrap_or_else(|_| "koi-host".to_string())
                .to_lowercase()
        );
        ...
        let dereg = deregister(
            if outcome.final_name.is_empty() {
                &full_name
            } else {
                &outcome.final_name
            },
            if outcome.final_host.is_empty() {
                &host_name
            } else {
                &outcome.final_host
            },
    ...
    pub fn run() {
                ...
                &[
                    (
                        ...
                        serde_json::Value::Array(
                            found.into_iter().map(serde_json::Value::from).collect(),
                        ),
                    ),
                    (
                        "terminals",
                        serde_json::Value::Array(
                            terminals.iter().map(|status| (*status).into()).collect(),
                        ),
                    ),
                ],
            );
        ...
        let Some(outcome) = register(&full_name, &host_name, probe_port, &txt, None) else {
            evidence(
                "summary",
                &[("result", "FAIL".into()), ("phase", "register".into())],
            );
            std::process::exit(1);
        };
        evidence(
        ...
        let self_seen = seen
            .iter()
            .any(|name| name == &full_name || name.starts_with(&format!("{probe_name}.")));
        evidence(
            "browse-self",
            &[
                (
                    "instances",
                    serde_json::Value::Array(
                        seen.iter().cloned().map(serde_json::Value::from).collect(),
                    ),
                ),
        ...
        let resolve_ok = resolved.as_ref().is_some_and(|service| {
            service.port == probe_port
                && service.txt.get("source").map(String::as_str) == Some("win32-dns-sd")
        });
        evidence(
            "resolve-self",
            &[
        ...
        let peer_resolve = peers
            .iter()
            .find(|name| !name.starts_with(&probe_name))
            .map(|peer| {
                let resolved = resolve(peer);
                evidence(
                    "resolve-peer",
                    &[
                        ("name", (*peer).clone().into()),
                        ("detail", resolved_json(&resolved)),
                    ],
                );
                resolved
        ...
        if peer_resolve.is_none() {
            evidence(
                "resolve-peer",
                &[("skipped", "no peer instance observed".into())],
            );
        }
        ...
        let gone = !after
            .iter()
            .any(|name| name.starts_with(&format!("{probe_name}.")));
        evidence(
            "deregister",
            &[
                ...
                (
                    "remaining",
                    serde_json::Value::Array(
                        after.iter().cloned().map(serde_json::Value::from).collect(),
                    ),
                ),
                (
                    "terminals",
...
fn main() {
    eprintln!("win32_dnssd_probe is available only on Windows");
}

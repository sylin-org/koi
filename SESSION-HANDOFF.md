...
The extended profile (ADR-035-era `profile_cases(Full)`: 16 Linux + 9 Windows-workstation cases, run elevated detached via `.tmp/full-profile.ps1`) passed after a shakedown that produced ADR-035 (gentle participation: detect/yield/declare/granted-latency/settlement) and its P1 implementation: assembly notes recorded by build_cores/serve merged into `/v1/status`'s capability ladder (now 8 rungs incl. `ipc` with skip reasons), Docker-watch scoping (`KOI_SCOPE` + `koi.scope` label — base daemon derives unlabeled only, scoped daemon derives exactly its scope), and capability-aware lane assertions. Shakedown fixes (all committed): catalog trust-store grant (operator-approved, `c564d0d`); native-trust Schannel settle-retry (`e92141b`, `aaf6d29` — the machine trust store settles asynchronously after a root import); W12 directory URL (`bfab513`) + cross-host dig lane port (`8315d40`); loud-guard sweep (RL-17: `fd2f7e5`, `bdd8d10`, `85085be`); transport refusals carry the command head (`a7a99ae`); W2 case package name (`f9ea64d`); W7 Schannel settle-retry (`0f7de9b`); cleanup transport retry (`cf364ff`); preserve-state port-guard narrowing (`3aaaa74`); reconnect/story lane-level isolation (`86e04b6`); service-lifecycle post-cutover adaptation (`345afad`) + precondition settle (`338d71c`) + prologue sweep (`85085be`); baseline compares service definitions not process identities (`359337a`). ENVIRONMENT (recorded, deliberate): systemd-resolved disabled on brook/granite (it held 5353/5355; DNS re-pointed to the gateway, verified); the operator's Windows koi service was uninstalled at operator direction so W1 could run in-profile (reinstall with `koi install` from target/release/koi.exe when wanted — W1 itself leaves no service behind). W1 rides in-profile only on machines without an operator install; it stays green standalone (2026-08-24 evidence stands).

**W10 GREEN — `certmesh-recovery-windows` (2026-08-27, run `v1-20260827T205903Z-e6881df9`, 8/8 checks incl. run_owned_cleanup). ADR-032 MATRIX COMPLETE: all lanes green (W11 excluded-by-tag). What remains for stable 1.0 is the gate itself: the extended full profile (fold W1/W2/W4 + the five windows-* lanes into `run-profile full`) run elevated end-to-end, then a clean soak incl. Windows participants.**
The lane was implemented this session (`36c0ed4`, ~450 lines + report model + CLI): CA hosted on the workstation through WindowsLabDaemon (trust_plane posture-reactive mTLS, HTTP+mtls scenario-scoped firewall rules), brook member joins with local CSR custody, then the full recovery arc from the Linux `certmesh-recovery` pattern: v2 encrypted backup → wrong-backup-passphrase prevalidation (live CA untouched) → exact data loss (daemon stopped, owned data root erased, restart uninitialized) → restore under a NEW passphrase → fingerprint + roster + machine-binding continuity → member renewal over restored mTLS → restart comes back LOCKED (no credential store), renewal refused with byte-identical member identity → old passphrase rejected, restored passphrase unlocks → second key-rotating renewal converges the roster → `backup_restored` in the CA audit log. Two physical runs: the FIRST passed every assertion and failed only in cleanup — `remove_windows_member_dir` compares the path by exact identity and refused the daemon's prefix-stripped root variant; fixed by removing via the constructed path (`1e75a19`, committed as a real defect fix, not a test tweak). Retry protocol held throughout (RL-17b): full lab cleanup → fresh deploy → fresh run id between attempts.

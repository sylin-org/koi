...
| V1-11 | Standard seed + SDK betas | **in progress - step 1 complete (spec + vector); step 2 scaffolded (read-side SDKs tested); enroll-side + publication pending** | Step 1: the language-neutral trust spec (`docs/reference/trust-protocol.md`, ADR-020-era) gained the **Agent-Door section** (8): pinned server-card shape, `_mcp._tcp`/DNS TXT discovery records, both sanctioned auth shapes (DAT loopback / ADR-026 principal over mTLS), and the SAN-name dialing rule. The card is pinned as `docs/reference/vectors/agent-door-card.json` and executed against the Rust impl by `koi-serve` `http::tests::agent_door_vector_matches_the_impl`. Step 2 scaffolded: `packages/ts` (`@sylin-org/koi-client` 0.1.0-beta.1, zero-dep ESM, node:test) + `packages/python` (`sylin-koi-client` 0.1.0b1, stdlib-only, unittest) - both drive status/health/certmesh-status/posture/server-card/events-SSE and execute the SAME Agent-Door vector as Rust (one shape, three languages); 5/5 tests green each against stub daemons. Owed: raw-CSR enroll-side surface in both SDKs; publication remains gated on stable 1.0.0 + explicit external authority. |
